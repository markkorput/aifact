...
4. Usage pattern:
   ...
   - Run the init script to generate a pointer file at project root
   - The pointer file references the desired harness within `.aifact/harnesses/`
...
7. **Activation mechanism: Pointer file at project root**
   - Rationale: Avoids overwriting existing harness directories at project root; pointer file (generated by init script) references the desired harness within `.aifact/harnesses/`; enables clean discovery without symlink conflicts
   - Alternatives considered: Symlinking harness folders to root (overwrites existing directories), Multiple pointer files per harness (unnecessary complexity)

8. **Top-level `.aifact/` files: README.md, AGENTS.md, init**
   - Rationale: README.md provides human-oriented documentation; AGENTS.md contains generic harness guidance (harness-specific implementations should ignore it and use their own); init script generates the pointer file for activation
