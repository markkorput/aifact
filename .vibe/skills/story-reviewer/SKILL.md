...
Review a drafted story before implementation-oriented analysis. Preserves artifact-first workflow.
...
- Tooling: you may use repo lookup (`read_file`, `grep`, `glob`) to ground discussion; only use write/modify tools for `work/`. Do not run `bash` commands that modify files.
- Handoff is never automatic: only hand off to another agent if the user explicitly asks for it.

## Purpose

Use this skill after a story has already been drafted.
...
1. **Read story from user input**
   - If no story path is provided, report error: "No story path provided"
...
4. **Perform minimal repo lookup in relevant code**
   - Start with one targeted lookup batch using the extracted keywords
   - Use `read_file`, `grep`, `glob` to find relevant files
   - Use `read_file` for the 1-3 most relevant files to confirm current architecture, patterns, and constraints
   - Stop searching once you can name:
     - The current pattern
     - The likely integration seam
     - The missing detail that materially affects the critique

5. **Produce critique**
   - Organize response using Output Contract sections
   - Use critique labels where relevant
...
This skill has tool access. Use it. Do not assume the repo state.
...
- If review cannot be completed due to missing context: report what's missing and stop
...
- Report the story path with critique output